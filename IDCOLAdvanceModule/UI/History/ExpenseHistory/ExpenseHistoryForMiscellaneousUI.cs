...
{
    public partial class ExpenseHistoryForMiscellaneousUI : Form
    {
        ...
        public ExpenseHistoryForMiscellaneousUI()
        {
        ...
        public ExpenseHistoryForMiscellaneousUI(long expenseHeaderId)
            : this()
        {
            var
        ...
        private void ExpenseHistoryForMiscellaneousUI_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void LoadFirstItem()
        {
            ...
            var expenseHistoryHeader =
               expenseHistoryListView.SelectedItems[0].Tag as MiscellaneousExpenseHistoryHeader;
            LoadData(expenseHistoryHeader);
        }

        private void LoadData(MiscellaneousExpenseHistoryHeader expenseHistoryHeader)
        {
            var expenseHeader = Mapper.Map<AdvanceMiscelleaneousExpenseHeader>(expenseHistoryHeader);
            LoadHeaderInformation(expenseHeader);
            var historyDetails = expenseHistoryHeader.MiscellaneousExpenseHistoryDetails.Select(c => (MiscellaneousExpenseHistoryDetail)c).ToList();
            LoadDetailsInformation(historyDetails);
        }

        private void LoadHeaderInformation(AdvanceMiscelleaneousExpenseHeader header)
        {
        ...
        private void LoadDetailsInformation(ICollection<MiscellaneousExpenseHistoryDetail> details)
        {
            try
            {
                expenseDetailsListView.Items.Clear();
                List<MiscellaneousExpenseHistoryDetail> deletedDetails = new List<MiscellaneousExpenseHistoryDetail>();
                foreach (MiscellaneousExpenseHistoryDetail detail in details)
                {
                    var expenseDetail = Mapper.Map<AdvanceMiscelleaneousExpenseDetail>(detail);
                ...
                if (deletedDetails.Count > 0)
                {
                    ...
                    foreach (MiscellaneousExpenseHistoryDetail detail in deletedDetails)
                    {
        ...
        private List<AdvanceMiscelleaneousExpenseDetail> GetAdvanceExpenseDetailsFromListView(ListView expenseDetailsListViewControl)
        {
            ...
            if (expenseDetailsListViewControl.Items.Count > 0)
            {
                ...
                {
                    var detailItem = item.Tag as MiscellaneousExpenseHistoryDetail;
                    if (detailItem != null)
                    {
                        var expenseDetail = Mapper.Map<AdvanceMiscelleaneousExpenseDetail>(detailItem);
